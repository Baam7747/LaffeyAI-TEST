...
export default {
    ...
    options: [
        {
            name: 'cancel',
            description: "Cancelling an ongoing deposit process!",
            required: false,
    ...
    callback: async ({ interaction }) => {
        ...
        userInfo.loadDatabase(async (err) => {    // Callback is optional
            ...
            userInfo.find({ discordID: discordid }, async (err: Error | null, docs: any[]) => {
                ...
                if (!interaction.options.getString('cancel')) {
                    ...
                    if ((docs[0] == undefined) || (docs[0].verification.verified == "false")) {
                        ...
                        interaction.editReply(`Deposit process started! Once you've made your deposit in-game, run this command again.\nPlease make sure to include the code and only the code below in your deposit note in-game`)
                    ...
                    { nations(id: ${docs[0].nationID}, first: 1)
                        ...
                        for (let i = 0; i < data.nations.data[0].bankrecs.length; i++) {
                            if ((data.nations.data[0].bankrecs[i].note === docs[0].deposit_code) && ((data.nations.data[0].bankrecs[i].recipient_id == 5476) || (data.nations.data[0].bankrecs[i].recipient_id == 8594) || (data.nations.data[0].bankrecs[i].recipient_id == 10382))) {
                                    ...
                                    userInfo.update({ discordID: discordid },
                                        {
                                            $set: {
                                                deposit_code: 0,
                                                deposits: {
                                                    money: money_round(money + money1),
                                                    coal: money_round(coal + coal1),
                                                    oil: money_round(oil + oil1),
                                                    uranium: money_round(uranium + uranium1),
                                                    lead: money_round(lead + lead1),
                                                    iron: money_round(iron + iron1),
                                                    bauxite: money_round(bauxite + bauxite1),
                                                    gasoline: money_round(gasoline + gasoline1),
                                                    munitions: money_round(munitions + munitions1),
                                                    steel: money_round(steel + steel1),
                                                    aluminum: money_round(aluminum + aluminum1),
                                                    food: money_round(food + food1)
                                                }
                                            }
                                        },
                                        { multi: false },
                                ...
                                return
                            } else if (i === data.nations.data[0].bankrecs.length - 1) {
                                let embed = new x.Embed()
                                    .setTitle('Error!')
                                    .setDescription("I don't see the code I gave you in your deposit notes...")
                ...
                } else if (interaction.options.getString('cancel') == ("true")) {
                    ...
                    userInfo.update({ discordID: discordid },
                        ...
                        interaction.editReply("Your deposit process has been cancelled!")

                } else if (interaction.options.getString('cancel') == ("false")) {
                    interaction.editReply(`Why you waste my time if you no cancel your deposit process <:yotsuBruh:986092690527633429>`)
                }
            })
        })
    }
